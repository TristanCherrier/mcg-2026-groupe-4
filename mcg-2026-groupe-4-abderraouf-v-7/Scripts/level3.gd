...
func _ready() -> void:
	...
	var ship_camera := ship_root.get_node_or_null("cam_pivot/SpringArm3D/Camera3D") as Camera3D
	if ship_camera != null:
...
func _physics_process(delta: float) -> void:
	...
	if ship_body.global_position.y < -3.0 or absf(ship_body.global_position.x) > 12.0 or absf(ship_body.global_position.z) > 12.0:
		_respawn_ship(true)
...
func _build_level() -> void:
	...
	# Reactor core et spirales supprimés
	# Fins latérales supprimées
	_make_crown_arch()
	_make_route_guides()
	...
	for y in [8.0, 16.0, 24.0, 32.0]:
		_make_light(Vector3(0, y, 0), Color(1.0, 0.95, 0.9), 1.0, 14.0)
...
func _make_reactor_core() -> void:
	...
	# Halos réduits à quelques repères clés
	for y in [7.0, 15.6, 23.8]:
...
func _respawn_ship(apply_penalty: bool) -> void:
	if apply_penalty:
		...
		GameState.push_message("Sortie de zone ! -50 points.", 2.0)
...
func _snap_ship_camera() -> void:
	pass  # Le cam_pivot (ship_cam.gd) se positionne automatiquement dans _ready.
