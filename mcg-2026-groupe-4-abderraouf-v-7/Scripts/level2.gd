...
var checkpoint_nodes: Array[Node3D] = []
...
func _ready() -> void:
	...
	var car_camera := car_root.get_node_or_null("cam_pivot/SpringArm3D/Camera3D") as Camera3D
	if car_camera != null:
...
func _build_level() -> void:
	...
	# Boîtes danger : disposées en chicane, forcent à zigzaguer
	for hazard_data in [
		{"size": Vector3(1.2, 1.4, 1.2), "position": Vector3(-3.5, 0.7, -20.0)},
		{"size": Vector3(1.2, 1.4, 1.2), "position": Vector3(3.5, 0.7, -26.0)},
		{"size": Vector3(1.2, 1.4, 1.2), "position": Vector3(-3.5, 0.7, -52.0)},
		{"size": Vector3(1.2, 1.4, 1.2), "position": Vector3(3.5, 0.7, -58.0)},
		{"size": Vector3(1.2, 1.4, 1.2), "position": Vector3(-3.5, 0.7, -76.0)},
		{"size": Vector3(1.2, 1.4, 1.2), "position": Vector3(3.5, 0.7, -82.0)}
	]:
		_make_hazard_box(hazard_data.size, hazard_data.position)

	# Obstacle A : se déplace de gauche à droite au milieu du couloir
	var moving_a := MOVING_OBSTACLE_SCENE.instantiate()
	moving_a.position = Vector3(0, 1.0, -35.0)
	moving_a.distance = 5.5    # amplitude = toute la largeur du couloir
	moving_a.speed = 1.4       # vitesse lisible
	...
	# Obstacle B : se déplace en sens inverse, plus rapide
	var moving_b := MOVING_OBSTACLE_SCENE.instantiate()
	moving_b.position = Vector3(0, 1.0, -65.0)
	moving_b.distance = 5.5
	moving_b.speed = 1.8
	moving_b.axis = Vector3(-1, 0, 0)   # sens inverse pour varier
	moving_b.add_to_group("hazard_obstacle")
	gameplay_root.add_child(moving_b)
	moving_obstacles.append(moving_b)

	# Obstacle C : plus lent mais centré pile avant l'arrivée
	var moving_c := MOVING_OBSTACLE_SCENE.instantiate()
	moving_c.position = Vector3(0, 1.0, -88.0)
	moving_c.distance = 4.5
	moving_c.speed = 1.1
	moving_c.axis = Vector3(1, 0, 0)
	moving_c.add_to_group("hazard_obstacle")
	gameplay_root.add_child(moving_c)
	moving_obstacles.append(moving_c)
...
func _on_score_changed(new_score: int) -> void:
	if _game_over or _level_completed:
		return
	if new_score <= -150:
		_game_over = true
		GameState.push_message("Score trop bas ! Retour au niveau 1...", 2.5)
...
func _on_car_body_entered(body: Node) -> void:
	...
	if body.is_in_group("hazard_obstacle"):
		...
		GameState.push_message("Collision ! -100 points. Retour au dernier checkpoint.", 2.5)
...
func _respawn_car(apply_penalty: bool) -> void:
	...
	# Remettre tous les checkpoints en orange
	checkpoints_hit = 0
	respawn_transform = Transform3D(Basis.IDENTITY, Vector3(0.0, 0.35, -8.8))
	for cp in checkpoint_nodes:
		if cp != null and cp.has_method("reset"):
			cp.reset()
	if apply_penalty:
	...
	if car_body.has_method("reset_vehicle"):
		car_body.call("reset_vehicle", respawn_transform)
	var car_camera := car_root.get_node_or_null("cam_pivot")
