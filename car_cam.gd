...
func _process(delta: float) -> void:
	target.global_position = car_rb.global_position + car_rb.global_basis.y * 0.6 + car_rb.global_basis.z * 0.8
	target.look_at(car_rb.global_position - car_rb.global_basis.z * 20, car_rb.global_basis.y)
